...
function GroupBooking() {
  ...
  // ### STATE ###
  const [instance, setInstance] = useState<BackendGroupMeetingInstance | null>(
    null,
  );
            ...
            (
              ...
              .filter(
                (b) =>
                  ...
                  proposalSlots.some(
                    (s: BackendBookingSlot) =>
                      s.bookingSlotID === b.bookingSlot.bookingSlotID,
                  ),
              )
      ...
      setSlots((prev) =>
        prev.map((s) => {
          ...
          return {
            ...s,
            slotStatus:
              newCount >= s.maxUsers ? ("BOOKED" as const) : s.slotStatus,
          };
        }),
      );
    } catch (err: unknown) {
  ...
  if (isLoading) {
    return (
      ...
      >
        <p style={{ color: "#8e8e8e", paddingTop: "60px" }}>
          Loading invite...
        </p>
      </div>
    );
        ...
        <Card>
          <Card.Content>
            <div style={{ textAlign: "center", padding: "40px 0" }}>
              ...
              >
                <strong>{instance.name}</strong> has been finalized by{" "}
              ...
              >
                This invite link is no longer active. If you were marked as
                available, the confirmed meeting should appear in your{" "}
                <strong>My Appointments</strong> on the dashboard.
              </p>
              <Button variant="primary" onClick={() => navigate("/dashboard")}>
                Go to Dashboard
              </Button>
    ...
    <div style={{ maxWidth: "800px", margin: "0 auto", padding: "40px 20px" }}>
      ...
      {isOwnLink && (
        ...
        >
          <span>🔒</span>
          <span>
            This is your own invite link. Share it with participants so they can
            mark their availability - you cannot mark availability on your own
            meeting.
          </span>
        </div>
      )}
      ...
      <div style={{ marginBottom: "40px" }}>
        ...
        <p style={{ color: "#8e8e8e", fontSize: "15px", margin: "0 0 12px" }}>
          Hosted by {instance.owner.firstName} {instance.owner.lastName}
        ...
        >
          ...
          <span>·</span>
          <span>
            📅 {slots.length} time option{slots.length !== 1 ? "s" : ""}
          </span>
        </div>
      </div>
      ...
      >
        Mark all the time options you are available for. The organizer will pick
        a final time based on responses.
      </div>
                    ...
                    <div style={{ display: "flex", alignItems: "center" }}>
                      {isOwnLink ? (
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            handleMarkAvailable(slot.bookingSlotID)
                          }
                          disabled={isMarking}
                        >
